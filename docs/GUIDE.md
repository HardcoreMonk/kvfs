...
- Ep.47~54 (P8 anti-entropy wave): self-heal + polish — `blog/47..54` (auto-repair → corrupt + dry-run → EC inline → EC corrupt → throttle/precision → concurrent EC → resilience polishes → auto-repair scheduling + Prometheus surface).

### 13.3 ADR

[`docs/adr/README.md`](adr/README.md) — **58 ADR** (S1~S6 + P8 + S7 close + P8-08~15 self-heal + concurrent + resilience polishes + auto-repair/metrics), 시즌별 표, 번호 오름차순. 결정의 근거가 모두 여기.

### 13.4 라이브 데모

`./scripts/demo-*.sh` — **47개** = α~ω (S1~S4, 그리스 21자) + aleph~nun (S5~S6, 히브리 14자) + samekh~tsadi (S7, 4자) + P8-08~15 anti-entropy specials (8개). ADR README 의 시즌 표에서 episode → 데모 매핑 확인.
