...
- **2~3 종류의 daemon**: 기본은 `kvfs-edge` + `kvfs-dn`, S5 이후 `kvfs-coord` 를 켜면 edge 는 thin gateway 가 된다.
...
- **운영성**: edge/coord 가 자기 cluster 의 정렬·청소·복구·snapshot·heartbeat·anti-entropy self-heal 을 **스스로** 한다.
...
- Counter / Gauge / Histogram. Histogram 은 ADR-063 에서 anti-entropy audit / repair duration 용으로 추가.
...
- Coord anti-entropy 가시성 (ADR-062/063): audits / repairs / throttled / unrecoverable / skipped counters + audit / repair duration histograms.
...
| `EDGE_S3_ACCESS_KEY` / `EDGE_S3_SECRET_KEY` | (off) | P9 S3 front-door credentials. Set together. |
| `EDGE_S3_REGION` | `us-east-1` | P9 S3 SigV4 credential-scope region. |
...
| `EDGE_ELECTION_HB_INTERVAL` | 500ms | leader heartbeat cadence |
| `EDGE_ELECTION_TIMEOUT_MIN` / `EDGE_ELECTION_TIMEOUT_MAX` | 1500ms / 3000ms | follower election timer jitter range |
...
| `EDGE_AUTO_GC_MIN_AGE` | 60s | GC 안전망 — 이보다 어린 chunk 보호 |
| `EDGE_AUTO_CONCURRENCY` | 4 | rebalance/GC worker 동시 실행 수 |
| `EDGE_SNAPSHOT_DIR` / `EDGE_SNAPSHOT_INTERVAL` / `EDGE_SNAPSHOT_KEEP` | (off) / 1h / 7 | 설정 시 auto-snapshot scheduler |
| `EDGE_HEARTBEAT_INTERVAL` / `EDGE_HEARTBEAT_FAIL_THRESHOLD` | 10s / 3 | DN heartbeat interval / unhealthy threshold (`0s` disables) |
...
| `EDGE_DN_TLS_CA` / `EDGE_DN_TLS_CLIENT_CERT` / `EDGE_DN_TLS_CLIENT_KEY` | (off) | edge → DN TLS / mTLS client config |
| `EDGE_SKIP_AUTH` | 0 | demo 전용 UrlKey bypass |
...
| `COORD_ANTI_ENTROPY_INTERVAL` | (off) | ADR-055: leader-only scheduled audit |
| `COORD_AUTO_REPAIR_INTERVAL/MAX/CONCURRENCY` | (off) / 100 / 4 | ADR-062: leader-only scheduled repair + burst cap + worker pool |
...
### 12.5 P8 — Frame-1+2 close + anti-entropy self-heal

P8 은 chaos / textbook primitive / anti-entropy polish wave. frame 1 (헌장) +
frame 2 (textbook primitives) 를 닫고, self-heal coverage 를 replication
missing/corrupt + EC missing/corrupt 4채널로 완성했다.

- Chaos hardening: Raft stale-log vote check + coord bootstrap snapshot pull (ADR-050).
- Season 7 textbook primitives: failure domain hierarchy (ADR-051), degraded read (ADR-052), tunable consistency (ADR-053), anti-entropy Merkle + scrubber (ADR-054).
...
- Ep.47~55 (P8 anti-entropy wave): self-heal + polish — `blog/47..55` (auto-repair → corrupt + dry-run → EC inline → EC corrupt → throttle/precision → concurrent EC → resilience polishes → auto-repair scheduling + Prometheus surface → observability completions).
...
`./scripts/demo-*.sh` — **48개** = α~ω (S1~S4, 그리스 21자) + aleph~nun (S5~S6, 히브리 14자) + samekh~tsadi (S7, 4자) + P8-08~16 anti-entropy specials (9개). ADR README 의 시즌 표에서 episode → 데모 매핑 확인.
