...
5. **Rebalance worker** ← **Ep.2 완료** — 정지된 청크를 desired DN 으로 이사 (copy-then-update, never-delete · ADR-010)
6. Chunking · Reed-Solomon EC · GC — 예정
...
#    → never-delete 규칙: dn1/2/3 의 surplus 청크는 그대로 (GC 는 별도 ADR)
```
...
| [010](docs/adr/ADR-010-rebalance-worker.md) | Rebalance worker | `internal/rebalance/`. copy-then-update, never-delete |

### Season 2+ 예정 (docs/FOLLOWUP.md)

- ADR-008 Reed-Solomon EC · ADR-011 Chunking
- ADR-012 Surplus 청크 GC · ADR-013 Coordinator daemon 분리 · ADR-014 gRPC 이행
...
- [`blog/03-rebalance.md`](blog/03-rebalance.md) — Rebalance worker · copy-then-update · never-delete 안전 규칙 (ADR-010)
