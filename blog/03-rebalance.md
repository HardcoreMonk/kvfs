...
- **Ep.4** *(완료)*: ADR-012 Surplus chunk GC — 이번 episode가 미룬 surplus 청크 청소. claimed-set + min-age 두 안전망. → [`blog/04-gc.md`](04-gc.md)
- **Ep.5**: ADR-013 Auto-trigger policy — DN 변경 감지 + rebalance + GC 자동화
- **Ep.6**: ADR-011 Chunking — 큰 객체 분할. rebalance·GC 가 chunk 단위로 자연 확장
- **Ep.7**: ADR-008 Reed-Solomon EC — placement가 (K+M)개 노드 선택. N ≥ K+M 필요. Season 2의 보스전
